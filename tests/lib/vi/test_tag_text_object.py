...
TESTS_SEARCH_TAG_FORWARD = (
    ...
    test_data(content='<a hey="ho">foo', args={'start': 0}, expected=(Region(0, 12), 'a', False), msg='find tag with attributes'),
)

TESTS_SEARCH_TAG_BACKWARD = (
    test_data(content='<a>foo', args={'pattern': r'</?(a) *?.*?>', 'start': 0, 'end': 6}, expected=(Region(0, 3), 'a', True), msg='find tag'),
    test_data(content='<a>foo', args={'pattern': r'</?(a) *?.*?>', 'start': 0, 'end': 0}, expected=(None, None, None), msg="don't find tag"),
    test_data(content='</a>foo', args={'pattern': r'</?(a) *?.*?>', 'start': 0, 'end': 6}, expected=(Region(0, 4), 'a', False), msg='find a closing tag'),
    test_data(content='<a>foo</a>', args={'pattern': r'</?(a) *?.*?>', 'start': 0, 'end': 5}, expected=(Region(0, 3), 'a', True), msg='find other tag'),
    test_data(content='<a hey="ho">foo', args={'pattern': r'</?(a) *?.*?>', 'start': 0, 'end': 14}, expected=(Region(0, 12), 'a', True), msg='find tag with attributes'),
)

TESTS_NEXT_UNBALANCED_END_TAG = (
    test_data(content='<p>foo <p>bar</p> baz</p>', args={'search': next_end_tag, 'search_args': {'start': 3}, 'restart_at': get_region_end}, expected=(Region(21, 25), 'p'), msg='find end tag skipping nested'),
)

TESTS_CONTAINING_TAG = (
    test_data(content='<a>foo</a>', args={'start': 4}, expected=(Region(0, 3), Region(6, 10), 'a'), msg='find tag'),
    test_data(content='<div>foo</div>', args={'start': 5}, expected=(Region(0, 5), Region(8, 14), 'div'), msg='find long tag'),
    test_data(content='<div class="foo">foo</div>', args={'start': 17}, expected=(Region(0, 17), Region(20, 26), 'div'), msg='find tag with attributes'),
    test_data(content='<div>foo</div>', args={'start': 2}, expected=(Region(0, 5), Region(8, 14), 'div'), msg='find tag from within start tag'),
    test_data(content='<div>foo</div>', args={'start': 13}, expected=(Region(0, 5), Region(8, 14), 'div'), msg='find tag from within end tag'),
    test_data(content='<div>foo <p>bar</p></div>', args={'start': 12}, expected=(Region(9, 12), Region(15, 19), 'p'), msg='find nested tag from inside'),
)
