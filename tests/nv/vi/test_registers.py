...
class TestCaseRegistersConstants(unittest.TestCase):
    ...
    def test_sys_clipboard_all_constant_value2(self):
        self.assertEqual(registers._SPECIAL,
                         (registers._UNNAMED,
                          ...
                          registers._FILE_NAME,
                          registers._ALT_FILE_NAME,
                          registers._SYS_CLIPBOARD_1,
                          registers._SYS_CLIPBOARD_2,))
    ...
    def test_all_constant_value(self):
        self.assertEqual(registers._ALL,
                         (registers._SPECIAL +
                          registers._VALID_NUMBERS +
                          registers._VALID_NAMES))
...
class TestCaseRegisters(unittest.ViewTestCase):
    ...
    def test_can_set_normal_registers(self):
        for name in registers._VALID_NAMES:
            self.regs.set(name, [name])

        for number in registers._VALID_NUMBERS:
    ...
    def test_can_get_file_name_register(self):
        fname = self.regs.get(registers._FILE_NAME)
        self.assertEqual(fname, [self.view.file_name()])

    def test_can_get_clipboard_registers(self):
        self.regs.set(registers._SYS_CLIPBOARD_1, ['foo'])
        self.assertEqual(self.regs.get(registers._SYS_CLIPBOARD_1), ['foo'])
        self.assertEqual(self.regs.get(registers._SYS_CLIPBOARD_2), ['foo'])

        self.regs.set(registers._SYS_CLIPBOARD_2, ['bar'])
        self.assertEqual(self.regs.get(registers._SYS_CLIPBOARD_1), ['bar'])
        self.assertEqual(self.regs.get(registers._SYS_CLIPBOARD_2), ['bar'])
