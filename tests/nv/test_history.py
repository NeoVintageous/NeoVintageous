...
class TestHistory(unittest.TestCase):
    ...
    def test_history_get_type(self):
        self.assertEqual(history_get_type(' '), _HIST_INVALID)
        self.assertEqual(history_get_type(''), _HIST_INVALID)
        self.assertEqual(history_get_type('foobar'), _HIST_INVALID)
        self.assertEqual(history_get_type('x'), _HIST_INVALID)
        self.assertEqual(history_get_type(':'), _HIST_CMD)
        self.assertEqual(history_get_type('='), _HIST_EXPR)
        self.assertEqual(history_get_type('>'), _HIST_DEBUG)
        self.assertEqual(history_get_type('?'), _HIST_SEARCH)
        self.assertEqual(history_get_type('/'), _HIST_SEARCH)
        self.assertEqual(history_get_type('@'), _HIST_INPUT)
        self.assertEqual(history_get_type('cmd'), _HIST_CMD)
        self.assertEqual(history_get_type('debug'), _HIST_DEBUG)
        self.assertEqual(history_get_type('expr'), _HIST_EXPR)
        self.assertEqual(history_get_type('input'), _HIST_INPUT)
        self.assertEqual(history_get_type('search'), _HIST_SEARCH)

    def test_char2type(self):
        self.assertEqual(_char2type(':'), _HIST_CMD)
        self.assertEqual(_char2type('>'), _HIST_DEBUG)
        self.assertEqual(_char2type('='), _HIST_EXPR)
        self.assertEqual(_char2type('@'), _HIST_INPUT)
        self.assertEqual(_char2type('/'), _HIST_SEARCH)
        self.assertEqual(_char2type('?'), _HIST_SEARCH)
        self.assertEqual(_char2type(''), _HIST_INVALID)
        self.assertEqual(_char2type(' '), _HIST_INVALID)
        self.assertEqual(_char2type('x'), _HIST_INVALID)
        self.assertEqual(_char2type('foobar'), _HIST_INVALID)

    def test_name2type(self):
        self.assertEqual(_name2type('cmd'), _HIST_CMD)
        self.assertEqual(_name2type('debug'), _HIST_DEBUG)
        self.assertEqual(_name2type('expr'), _HIST_EXPR)
        self.assertEqual(_name2type('input'), _HIST_INPUT)
        self.assertEqual(_name2type('search'), _HIST_SEARCH)
        self.assertEqual(_name2type(''), _HIST_INVALID)
        self.assertEqual(_name2type(' '), _HIST_INVALID)
        self.assertEqual(_name2type('x'), _HIST_INVALID)
        self.assertEqual(_name2type('foobar'), _HIST_INVALID)
    ...
    def test_all(self, _storage):
        ...
        with self.assertRaises(TypeError):
            history_get([])  # type: ignore[arg-type]
        self.assertEqual('', history_get(10))  # type: ignore[arg-type]
